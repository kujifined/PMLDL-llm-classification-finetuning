...
\definecolor{optional}{HTML}{6B7280}
...
\midrule
Данил — TeamLead & E9 --- ensemble/blending & E8, E10 \\
Данил Куджи — ML Engineer & E1 --- pretrained transformer & E4 \\
Карим — ML Engineer & E2 --- LoRA/QLoRA и fine-tuning & E6 \\
Аслан — ML Engineer & E3 --- A/B-симметричная архитектура & E5 \\
Никита — Backend Engineer & B0 --- class-prior baseline; B1 --- structural baseline & --- \\
Арсений — Backend Engineer & B2 --- TF-IDF baseline & B3, E7 \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.5em}
\noindent\textcolor{optional}{\textit{Дополнительные задания необязательны. Их можно выполнить после завершения следующего stage, если останутся время и ресурсы. В Sprint 1 приоритет имеют основные задания и baseline B0--B3.}}
...
Это минимальная контрольная точка, с которой сравниваются все остальные решения.

\experiment{B1}{Structural baseline}
Использовать длины prompt и ответов A/B, количество слов и токенов,
соотношение длин, специальные символы и признаки пустых ответов. Поверх
признаков обучить Logistic Regression.

\experiment{B2}{TF-IDF baseline}
Преобразовать prompt и ответы A/B в word-level и character-level TF-IDF-признаки.
Обучить Logistic Regression или Linear SVM с вероятностным выводом.
...
победа модели B или tie. Можно выбрать DeBERTa, RoBERTa, ModernBERT или аналогичную модель.

\experiment{E2}{LoRA/QLoRA fine-tuning}
Сравнить полное дообучение с LoRA и QLoRA. Исследовать rank, dropout,
learning rate и количество обучаемых параметров.

\experiment{E3}{A/B-симметричная архитектура}
Пропускать ответы A и B через общий encoder отдельно. Использовать признаки
\code{h\_A-h\_B}, \code{|h\_A-h\_B|} и признаки prompt, чтобы модель была
устойчива к перестановке ответов.
...
Сравнить head-only, tail-only, head-tail и turn-aware truncation. Проверить,
какая стратегия лучше сохраняет информацию в длинных диалогах.

\experiment{E6}{Функция потерь}
Сравнить обычную cross-entropy, label smoothing, class weights и focal loss.
Цель --- проверить влияние loss при дисбалансе классов.

\experiment{E7}{Калибровка вероятностей}
Применить temperature scaling или vector scaling после обучения. Оценивать
изменение качества вероятностей по основной метрике \code{log\_loss}.

\experiment{E8}{Test-time augmentation}
Использовать несколько вариантов truncation или окон одного диалога во время
inference и усреднять итоговые вероятности.

\experiment{E9}{Blending моделей}
Смешать вероятности TF-IDF, embeddings и transformer-моделей.
Сравнить простое усреднение с подбором весов.

\experiment{E10}{Устойчивость к seed}
Запустить одну конфигурацию с несколькими seed, усреднить вероятности и оценить
средний результат и разброс качества.

\section*{Общие правила}

\begin{itemize}[leftmargin=*,itemsep=0.35em]
  \item Для каждого эксперимента менять одну основную гипотезу.
  \item Использовать общий frozen split и одинаковый evaluation protocol.
  \item Основная метрика сравнения --- \code{log\_loss}; дополнительные метрики сохраняются автоматически.
  \item Каждый результат запускать через единый pipeline и сохранять в ClearML.
\end{itemize}
