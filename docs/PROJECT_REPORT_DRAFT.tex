...
\toprule
Часть данных & Folds & Примеров & Назначение \\
\midrule
Обучение & 0--6 & 40\,235 & Обучение моделей. \\
Выбор кандидатов & 7 & 5\,746 & Сравнение baseline-моделей, абляции и выбор состава ансамбля. \\
Калибровка & 8 & 5\,748 & Подбор температуры и весов уже выбранных моделей. \\
Финальная оценка & 9 & 5\,748 & Однократная оценка только после фиксации конфигурации. \\
\bottomrule
\end{tabularx}
\end{table}
...
\toprule
Модель & \metric & Accuracy & Macro F1 \\
\midrule
Structural Logistic Regression & 1.06303 & 0.44692 & 0.44014 \\
Word + character TF-IDF & 1.04052 & 0.46920 & 0.46153 \\
Улучшенный sparse TF-IDF & 1.03576 & 0.47355 & 0.46595 \\
\bottomrule
\end{tabularx}
\end{table}
...
Параметр & Значение \\
\midrule
Backbone & \texttt{microsoft/deberta-v3-base}, revision \texttt{8ccc9b6f...}. \\
Способ обучения & QLoRA в 4-bit NF4; double quantization. \\
Адаптеры & $r=16$, $\alpha=32$, dropout $=0.05$; \texttt{query\_proj} и \texttt{value\_proj}. \\
Обучаемые модули & Адаптеры, \texttt{pooler} и \texttt{classifier}. \\
Длина входа & Максимум 512 токенов. \\
Оптимизация & AdamW, learning rate $2.8\cdot10^{-4}$, weight decay $0.01$, 3 эпохи. \\
Batch и scheduler & Effective batch size 32; warmup 0.06; scheduler horizon 5 эпох. \\
Симметрия A/B & Swap augmentation при обучении и усреднение вероятностей при inference. \\
\bottomrule
\end{tabularx}
\end{table}
...
\toprule
Эксперимент & Изменение & \metric & Изменение log loss \\
\midrule
Sparse TF-IDF sweep & Последовательный подбор $n$-грамм, словаря, \texttt{min\_df} и $\alpha$. & 1.03576 & $-0.00476$ относительно B2. \\
DeBERTa QLoRA, seed 42 & Выбор лучшего seed при неизменной QLoRA-конфигурации. & 1.01505 & Лучший отдельный seed. \\
Среднее трёх seed & Усреднение seed 17, 42 и 73. & 1.01889 & $+0.00384$ относительно seed 42. \\
\bottomrule
\end{tabularx}
\end{table}
...
\toprule
Конфигурация & \metric & Accuracy & Macro F1 & Решение \\
\midrule
Seed 17 & 1.02800 & 0.46711 & 0.46777 & Не выбран. \\
Seed 42 & 1.01505 & 0.48747 & 0.48420 & Выбран для ансамбля. \\
Seed 73 & 1.02807 & 0.47355 & 0.47328 & Не выбран. \\
Среднее трёх seed & 1.01889 & 0.48016 & 0.48001 & Уступает seed 42. \\
\bottomrule
\end{tabularx}
\end{table}
...
\small
\begin{tabularx}{\textwidth}{@{}p{0.35\textwidth}>{\raggedleft\arraybackslash}p{0.15\textwidth}>{\raggedleft\arraybackslash}p{0.14\textwidth}>{\raggedleft\arraybackslash}p{0.14\textwidth}>{\raggedleft\arraybackslash}p{0.12\textwidth}@{}}
\toprule
Модель & \metric & Accuracy & Macro F1 & ECE \\
\midrule
Калиброванная DeBERTa QLoRA & 1.00892 & 0.48713 & 0.48349 & 0.01833 \\
Калиброванный sparse TF-IDF & 1.02609 & 0.49791 & 0.49214 & 0.03329 \\
Финальный ансамбль & \textbf{0.99697} & \textbf{0.50035} & \textbf{0.49521} & \textbf{0.01111} \\
\bottomrule
\end{tabularx}
\end{table}
...
\toprule
Веса (DeBERTa, sparse) & Температура & \metric & ECE \\
\midrule
0.25, 0.75 & 0.87399 & 1.00397 & 0.01542 \\
0.50, 0.50 & \textbf{0.81731} & \textbf{0.99697} & \textbf{0.01111} \\
0.75, 0.25 & 0.84529 & 0.99899 & 0.01746 \\
\bottomrule
\end{tabularx}
\end{table}
...
Артефакт & Назначение \\
\midrule
\code{configs/split.json} и \code{data/splits/folds.csv} & Фиксированные роли folds и назначение примеров. \\
\code{configs/experiments/} & Конфигурации retained baseline, sparse и DeBERTa кандидатов. \\
\code{configs/final_model.json} & Замороженные модели, веса $0.5/0.5$ и температура $0.817311$. \\
\code{results/model_comparison.csv} & Компактная таблица результатов, используемых в отчёте. \\
\code{results/final/} & Evidence выбора кандидатов, перебора весов и калибровки. \\
\code{artifacts/FINAL_ARTIFACTS.md} & SHA-256 и расположение двух внешних чекпойнтов. \\
\code{docs/REPRODUCE.md} & Порядок восстановления артефактов и запуска финального pipeline. \\
\code{requirements*.lock} & Зафиксированные зависимости. \\
\bottomrule
\end{tabularx}
\end{table}
